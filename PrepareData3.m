function [data, target, fs]= PrepareData3(directory,fw, fi,persons, tracks, delS, delta, deltadelta)
...
[mMFCC, nMFCC]= size(mfccData);
[mLPC,nLPC]= size(lpcData);
zeroMfccData= zeros(mMFCC,nMFCC);
zeroLpcData=zeros(mLPC,nLPC);
mfccFinal= [mfccData,zeroMfccData,mfccData];
lpcFinal= [zeroLpcData, lpcData,lpcData];
data= {mfccFinal;lpcFinal};
target= cell2mat(target);
target= repmat(target,1,3);